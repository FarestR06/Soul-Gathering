...
public class SoulComponentHelper {
    ...
    public static int calcSoulAdderAmount(PlayerEntity provider) {
        Map<String, Integer> map = ModConfig.ITEMS_AND_GATHERING;
        int amount = 0;
        for (ItemStack itemStack : provider.getArmorItems()) {
            if (itemStack.getComponents().contains(SoulDataComponentTypes.SOUL_GATHERING)) {
            ...
            } else if (map.containsKey(itemStack.getItem().toString())) {
                amount += map.get(itemStack.getItem().toString());
            }
        ...
        amount = MathHelper.floor(amount * Random.create().nextFloat());
        return amount;
    }
